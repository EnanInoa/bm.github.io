...
<body>
 ...
 <label for="checkbox-menu" class="icono-menu"><img src="../iconos/menu.svg" alt=""></label>
  ...
  $matricula = $_GET['matricula'];
    ?>
    <form  action="procesarAgregarProfesor.php" method="get">

    <div class= "contenedor-agregar">
      <h2 class="form_titulo">Agregar Profesor</h2>
    ...
    <label for="grado">Titular de grado</label>
    <select name="grado" id="grado">
    <option value="" disabled selected>Titular De Grado</option>
<?php
$consulta = mysqli_query($conexion, "SELECT * FROM grado WHERE grado.idGrado = 0  OR grado.idGrado NOT IN(SELECT id_grado_titular FROM profesores WHERE id_grado_titular != '$idGrado')");
while ($mostrar = mysqli_fetch_assoc($consulta)) {

?>

     <option value="<?php echo $mostrar['idGrado']?>"><?php echo $mostrar['nombreGrado']?></option>
<?php
}
?>
